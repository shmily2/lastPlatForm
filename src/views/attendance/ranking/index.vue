<template>
  <div class="attendance-ranking-page">
    <!-- 页面标题 -->
    <div class="page-header">
      <h2 class="page-title">学生打卡 / 打卡排名</h2>
    </div>
    ...
    <el-card class="chart-card">
      ...
      <div v-show="viewMode === 'table'" class="table-container">
        <el-table :data="rankingData" border stripe>
          ...
          <el-table-column prop="rate" label="打卡率(%)" min-width="150" sortable>
...
const rankingData = ref([
  ...
  { className: '护理2020级', rate: 8, total: 45, clocked: 4, unclocked: 41 },
  ...
  { className: '护理2026级', rate: 6, total: 45, clocked: 3, unclocked: 42 },
  ...
  { className: '临床2021级', rate: 5, total: 50, clocked: 2, unclocked: 48 },
...
const initChart = () => {
  ...
  const option = {
    ...
    xAxis: {
      type: 'category',
      data: rankingData.value.map(item => item.className),
      axisLabel: {
        interval: 0,
        rotate: 30,
        fontSize: 11
    ...
    series: [
      {
        name: '打卡率',
        type: 'bar',
        data: rankingData.value.map(item => item.rate),
        barWidth: '40%',
        itemStyle: {
          color: (params) => {
